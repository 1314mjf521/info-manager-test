...
function Login {
    ...
    try {
        $response = Invoke-RestMethod -Uri "$BaseUrl/api/v1/auth/login" -Method POST -Body $loginData -Headers $global:Headers
...
function Test-GetPermissionTree {
    ...
    try {
        ...
        if ($response.success) {
            ...
            Write-Host "权限树结构:" -ForegroundColor Cyan
...
function Test-AssignPermissions($roleId, $permissions) {
    Write-Host "6. 测试分配权限..." -ForegroundColor Yellow
    
    # 获取前几个权限ID进行测试
    $permissionIds = @()
    if ($permissions -and $permissions.Count -gt 0) {
        $permissionIds = $permissions[0..([Math]::Min(2, $permissions.Count - 1))] | ForEach-Object { $_.id }
    }
    ...
    try {
        $response = Invoke-RestMethod -Uri "$BaseUrl/api/v1/roles/$roleId/permissions" -Method PUT -Body $permissionData -Headers $global:Headers
...
function Test-GetRolePermissions($roleId) {
    Write-Host "7. 测试获取角色权限..." -ForegroundColor Yellow
...
function Test-DeleteRole($roleId) {
    Write-Host "8. 测试删除角色..." -ForegroundColor Yellow
...
function Main {
    ...
    # 2. 获取权限树
    $permissions = Test-GetPermissionTree
    Write-Host ""
